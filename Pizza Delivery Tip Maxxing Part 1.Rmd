---
title: "Pizza Delivery Tip-Maxxing Project"
subtitle: Saumi Rahnamay, 2022
output: github_document
---

# Preface

This project originally started as an assignment for POL 315 at SFU. I returned to it later in 2022 to fully flesh it out. The result is the first part of an ongoing project to analyze data surrounding my part time job as a pizza delivery driver.
...
```{r prelim1, message = FALSE, warning=F}
rm(list = ls())
library(readxl)
...
```{r prelim2, message = FALSE, warning=FALSE}
## Sales Receipts
library(plyr)
...
[^2]: Observation 37 had a TipSum value of 0, which means I could not take the log of the variable without dealing with the zero. I decided to drop the observation because a \$0 cash-out is very unlucky, and is almost certainly an outlier in our data set.
...
```{r sec3-biv, message = FALSE, tidy=T, warning=FALSE}
...
```{r sec3-biv-graphic, message = FALSE, warning=FALSE}
library(ggplot2)
library(ggthemes)
...
```{r sec3-multiple, message = FALSE, warning=FALSE}
...
```{r sec4-lin, message = FALSE, warning=FALSE}
library(car)
...
```{r sec4-add, message = FALSE, warning=FALSE}
...
```{r sec4-homosked, message = FALSE, warning=FALSE}
#Normality
...
```{r sec4-multicol, message = FALSE, warning=FALSE}
vif(logdlmm)
vif(logwlmm)
```
...
```{r sec4-revised-model, message = FALSE, warning=FALSE}
tppP <- tpp[-c(53, 27), ]
